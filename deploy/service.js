...
/**
 * Deployment tool for website services
 * Requirements:
...
const image = `basecmswebsites/${service}-service`;
...
const build = async () => {
  ...
  const { status } = await spawnSync('bash', ['deploy/service-image.sh', service, version], { stdio: 'inherit' });
  if (status !== 0) error('Image build failed!');
};

const deploy = async () => {
  log(`Deploying ${image}:${version} on Kubernertes`);
  const { status } = await spawnSync('bash', ['deploy/service-k8s.sh', service, version, namespace], { stdio: 'inherit' });
